...
public class Util {
    ...
    public static void renderItemGroup(MatrixStack matrixStack, IRenderTypeBuffer buffer, NonNullList<ItemStack> stacks, int rows, int itemsPerRow, ItemGroup group) {
        RenderHelperMod.renderDefaultBackground(matrixStack, buffer, itemsPerRow * 18 + 8, rows * 18 + 24);
        RenderHelperMod.renderItem(matrixStack, buffer, group.getIcon(), 5, 3);
        ...
        for (int y = 0; y < rows; y++) {
            for (int x = 0; x < itemsPerRow; x++) {
                ...
                RenderHelperMod.renderSlot(matrixStack, buffer, 5 + x * 18, 21 + y * 18);
                if (stacks.size() > idx) {
                    RenderHelperMod.renderItem(matrixStack, buffer, stacks.get(idx), 5 + x * 18, 21 + y * 18);
                }
            }
        }
    }
